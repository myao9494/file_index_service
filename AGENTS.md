...
## 2. 開発基本ルール
- **仕様とコードの同期**: コードを変更する際は、必ずこの `AGENTS.md` および `docs/` 内のドキュメントも更新すること。
...
### 主要コンポーネント (Frontend)
- `App.tsx`: メインアプリケーション。
- `components/`: ステータス表示、監視パス管理、除外設定、検索テストUI。

## 6. データストレージ仕様

### データベース
- **場所**: `backend/data/file_index.db`
- **形式**: SQLite (FTS5 + Trigram Index + Bigram Index)
- **永続化仕様**:
  - アプリケーション起動時にファイルが存在しない場合、**自動的に空のデータベースを作成**します。
  - 親ディレクトリ (`backend/data`) も自動生成されます。
  - Git管理対象外 (`.gitignore` に記載済み) です。

### 復旧・移行
- フォルダー移動などでDBファイルが失われた場合、次回起動時に空の状態で再初期化されます。
- 既存のDBを引き継ぐ場合は、手動で `file_index.db` をコピーしてください。

## 7. 起動スクリプト
- **本番/PWA環境 (`start.sh`)**:
  - フロントエンドをPWAとしてビルド(`npm run build`)し、成果物をバックエンドの `static` ディレクトリに配置します。
  - バックエンドのみを起動(ポート8080)し、バックエンドからフロントエンドの静的ファイルを配信します。
  - PWAのインストール(`manifest.webmanifest` / Service Worker)に完全対応しています。

- **開発環境 (`start_dev.sh`)**:
  - バックエンド(ポート8080)とフロントエンド開発サーバー(Vite: ポート5174)を同時に並列起動します。
  - 開発中のHMR(Hot Module Replacement)などを利用するためのスクリプトです。

- **自動修復**:
  - スクリプト起動時に `backend/.venv` が破損または存在しない場合、自動的に削除して再作成 (`python3 -m venv`) し、`requirements.txt` をインストールします。
  - フロントエンドの `node_modules` が無い場合は、自動で `npm install` を実行します。
- **ポート競合**:
  - 使用するポート (8080 または 5174) が使用中の場合、対話的にプロセスを終了させるか確認します。
